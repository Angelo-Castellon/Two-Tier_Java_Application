...
public class AccountantInterfaceGUI extends JFrame {
    ...
    public AccountantInterfaceGUI(JDBCHandler jdbcHandler, int width, int height) {
        this.jdbcHandler = jdbcHandler;
        setTitle("SQL CLIENT APPLICATION - ACCOUNTANT VIEW");
        ...
        urlPropertiesPanel.add(new JLabel("DB URL Properties"));
        JTextField urlField = new JTextField("operationslog.properties");
        urlField.setEditable(false); // Make it uneditable
        urlPropertiesPanel.add(urlField);
        ...
        userField.setEditable(false); // Make it uneditable
        userPropertiesPanel.add(userField);
...
// Close Application button
        ...
        connectButton.addActionListener(e -> {
            String urlProp = urlField.getText(); // URL from the urlField
            String userProp = userField.getText(); // User property from the userField

            try {
                String url = jdbcHandler.connect(urlProp, userProp); // Assuming this returns the connection URL
                statusLabel.setText("CONNECTED TO: " + url); // Display the connection URL
            } catch (Exception ex) {
                statusLabel.setText("CONNECTION FAILED"); // Display failure message
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    for (int i = 1; i <= columnCount; i++) {
                        ...
                        if (columnName.equalsIgnoreCase("login_username")) {
                            // Modify the login_username column to append @localhost
                            rowData[i - 1] = rs.getString(i).replace(".properties", "@localhost");
                        } else {
